...
## Per-kid context profile

A short, evolving markdown document for each kid that captures
what they've explored, what they seem interested in, and how they
ask questions. Auto-generated by the AI from the kid's activity,
prepended to every AI prompt as background.

### Why

The AI's main weakness as a mentor is that every conversation
starts from zero. A good mentor remembers — "you were asking about
volcanoes last week; have you noticed that earthquakes happen near
the same places?". Without some form of persistent context, the AI
can only respond to the current question in isolation.

A profile gives the AI a coarse sense of the kid:

- What topics they've returned to
- What level of detail they engage with
- What kinds of questions they tend to ask (curious vs. concrete vs.
  philosophical)
- Recently completed reading / videos
- A note about reading skill if it's been inferred from interactions

### Sources of input

The profile synthesizer reads from three sqlite logs that already
exist in the architecture:

- `state/searchd/queries.sqlite` and `clicks.sqlite` — what the kid
  searched for, what they opened.
- `state/aigateway/transcripts.sqlite` — what they asked the AI,
  how the AI responded.
- `state/launcher/activity.sqlite` — tile taps, time spent on each
  service. (New table, written by the launcher.)

None of these sources are new for the profile — they all exist
already for parental review and ranking history. The profile is a
*synthesizer over* them, not a fourth log.

### How it's generated

A periodic job (default: nightly during the maintenance window
when the AI is otherwise idle) runs:

```
1. Load existing profile (if any) for the kid.
2. Pull events from the last 7 days from the three log sources.
3. Render a synthesizer prompt:
     "Here's an existing profile for {kid_name}, age {age}.
      Here are their activities this week: <events>.
      Update the profile. Keep it under 200 tokens. Bias toward
      durable interests; let one-off questions decay. Never
      include real names other than the kid's, never speculate
      about emotions or family situation."
4. The local LLM produces the new profile as plain markdown.
5. Validate (length, banned content), write to
   state/aigateway/profiles/<kid_id>.md.
```

The synthesizer is the same Ollama model used for chat — there's
no second model. The prompt is in
`aigateway/prompts/profile_synthesize.md`, version-controlled.

### Use at query time

When a kid talks to the AI, the gateway prepends the profile to the
system prompt:

```
You are a curious learning companion for {kid_name}, age {age}.

Background on {kid_name} (do not bring this up unless directly
relevant to the question; never recite it back to them):
...
The `do not bring this up unless directly relevant` instruction is
critical — without it, models often open with "I see you've been
interested in volcanoes!", which is creepy. The profile is for the
model's reference, not the kid's awareness.
...
## Recurring interests
- Volcanoes and how the Earth works (4 conversations, 6 article
  reads in the past 3 weeks)
- Greek mythology (started this week)

## How she asks
- Tends to ask "why" questions ("why does lava cool?")
- Comfortable with chains of reasoning; can hold a 2- or 3-step
  explanation
- Skips ahead from concrete examples to general principles

## Reading
- Finished Wikipedia article on Mount Vesuvius
- In progress: Khan Academy unit on plate tectonics
- Has not yet engaged with the Kolibri Greek myths channel
  (started searching but didn't open results)

## Notes
- Asks follow-ups; appears to enjoy multi-turn conversations
- Has not asked any off-topic questions in the logged window
```

### Defenses against the obvious failure modes

- **Self-reinforcing loop.** If the profile says "loves dinosaurs",
  the AI keeps recommending dinosaurs, the profile keeps saying
  "loves dinosaurs". Mitigation: profiles age out — interests with
  no engagement in 30 days drop from "recurring" to "past"; nothing
  is permanent. The synthesizer prompt explicitly asks for "what's
  *recently* recurring", not "what they ever liked".
- **Drift from reality.** AI summaries can fabricate. Mitigation:
  the raw logs are the ground truth; the profile is regenerable
  from scratch any time. The parent surface (below) has a
  "regenerate from raw log" button.
- **Personalization fatigue.** A kid being constantly responded to
  *as their profile* is creepy. Mitigation: the system prompt
  forbids reciting it; the AI can use the profile to *inform* its
  questions, not display it back.
- **Sensitive inference.** A kid asks several questions that *could*
  be inferred as worry about a parent's illness. The profile must
  not contain anything like that. Mitigation: synthesizer prompt
  has a hard "never speculate about emotions or family situation"
  rule, and the profile is reviewed by a parent surface (next
  section) where they can edit or delete entries.
- **Profile becomes the prompt.** A long profile crowds out the
  retrieved sources. Mitigation: 200-token cap on profile content;
  enforced at synthesizer step.

### Parental review and override

`admin.kids/profile/<kid_id>` shows:

- The current profile, rendered.
- A "regenerate from raw log" button.
- A free-text override that the parent can edit ("Alice is moving
  house next week; please don't bring up moving topics") — appended
  to the profile as a "Parent notes" section that takes precedence
  in the prompt.
- A "clear profile" button. Resets to nothing; the AI loses memory.
- A toggle in `kids.yml` to disable profiles entirely for a kid:

  ```yaml
  permissions:
    ai_companion: true
    ai_profile: false      # AI doesn't get any background on this kid
  ```

  Default: `ai_profile: true` for ages ≥ 6, `false` below that.
  (Younger kids' interests are too volatile for the profile to be
  useful, and the personalization is more likely to feel weird.)

### Storage

Profiles live in `state/aigateway/profiles/<kid_id>.md`. Plain
markdown, plain filenames. Backed up nightly via restic. No
database — debugging is `cat`. A second-source-of-truth events
table is intentionally avoided; the synthesizer reads from the
existing logs every time.
